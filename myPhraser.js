FilesList = new Mongo.Collection('files');
...
if (Meteor.isClient) {
    ...
    Template.prompt.events({
      'submit form': function(event){
            ...
            inputUrl = event.target.inputUrl.value;
 
            Meteor.call('getFile', inputUrl, function(error, response) {
                Session.set('getFileResult', response);
            });
        }
    });
    Template.prompt.helpers({
        'file': function(){
            return Session.get('getFileResult');
        }
...
if (Meteor.isServer) {
    ...
    Meteor.methods({
        ...
        'pairCounter': function(doc, url){
            ...
            var finalData = Meteor.call('save',arranged,url);
            return finalData;
        },
        'arrify': function(str){
        ...
        'save': function(arr, url){                                      //prints out the phrases and the count for each phrase
            //console.log('arr',arr)
            var objArray = [],
            returnArray = [];
